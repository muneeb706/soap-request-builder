"""
Module containing constants used across the request builder.

This module defines common mappings and namespaces that are reused
in different parts of the request building process.

LANGUAGE_MAPPINGS:
   A dictionary mapping language codes to full language names.

NAMESPACES:
   A dictionary of namespaces used in the SOAP requests.

CustomerInformationAttributes:
   An enum of JSON keys for customer details.

ComplianceAttributes:  
   An enum of JSON keys for compliance details.
"""

from enum import Enum

# Mappings for language code to language
LANGUAGE_MAPPINGS = {"en": "English", "fr": "French"}
...
class CustomerInformationAttributes(Enum):
    ...
    @staticmethod
    def values():
        return list(map(lambda c: c.value, CustomerInformationAttributes))
